...
public class F3bTimerActivity extends AppCompatActivity {
    ...
    private void exitDialog() {
        ...
        MyAlertDialog.setTitle("Exit F3B Timer");
    ...
    private void onButtonStart() {
        ...
        if(mTimerState == idleState) {
            ...
            mCourseCount.set(-1);
            mTextViewTimerStatus.setText("Wait for speed course");
        }
    }

    private void onButtonStop() {
        if(mTimerState == thirtySecondState || mTimerState == speedCourseState) {
    ...
    private void onBaseA() {
        if(mTimerState == thirtySecondState) { // Enter speed course
            ...
            if (mCourseCount.get() < 4) {
                ...
                Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(millis < 15000) // Less than 15 seconds
                            toneArray.add("smb_world_clear.raw");
                        else if(millis < 20000) // Less than 20 seconds
    ...
    void onBaseTrigger(DragonEyeBase b, String s) {
        ...
        if(base == 'A' && serNo != serNoA) {
            if(mRepeatTriggerTick.get() == 0) {
                ...
                F3bTimerActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        onBaseA();
                    }
